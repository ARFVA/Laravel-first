    ...
    <div x-show="openDeleteModal" x-transition class="fixed inset-0 flex items-center justify-center bg-black/50 z-50">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg w-full max-w-md p-6 relative">
            ...
            <div class="text-center">
                ...
                <p class="mb-4 text-gray-500 dark:text-gray-300">Are you sure you want to delete this subject?</p>

                <div class="flex justify-center items-center space-x-4">
                    <button @click="openDeleteModal = false" type="button" class="py-2 px-3 text-sm text-gray-500 bg-white rounded-lg border hover:bg-gray-100 dark:bg-gray-700 dark:text-gray-300">No, cancel</button>

                    <form :action="deleteUrl" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="py-2 px-3 text-sm text-white bg-red-600 rounded-lg hover:bg-red-700">Yes, I'm sure</button>
                    </form>
                </div>
            </div>
        </div>
